...
const createCard = ({author, offer}) => {
  ...
  const hiddenElement = (nameOfClass) => {
  ...
  if (offer.address) {
    ...
    hiddenElement('text--address');
  }

  if (offer.price) {
    addText('text--price', `${offer.price} ₽/ночь`);
  } else {
    hiddenElement('text--price');
  }
